...
public class DownloadThread extends Thread {
	private static final String TAG = "DownloadThread";    //定义TAG,在打印log时进行标记
	private final File saveFile;              //下载的数据保存到的文件
	private final URL downUrl;              //下载的URL
	private final int block;                //每条线程下载的大小
	private final int threadId;            //初始化线程id设置a
	private int downLength;             //该线程已下载的数据长度
	private boolean finish = false;         //该线程是否完成下载的标志
	private final FileDownloadered downloader;      //文件下载器
